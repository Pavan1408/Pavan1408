...
def test_classes_are_objects():
    ...
    assert 'a' == Queue.__doc__
    assert 2 == get_attr_count(Queue)
...
def test_classes_are_object_factories():
    ...
    assert 'class' == type(q1).__class__
    assert 'class' == type(q2).__class__

    assert False  == (q1 is Queue)
    assert False  == (q2 is Queue)
    assert True  == (q2 is q1)
...
def test_classes_init_with_args():
    ...
    try:
        q3 = Queue()
    except TypeError as ne: #what error do you get?
        pass
...
def test_classes_bound_and_unbound_methods():
    ...
    assert None == q1_push     #bound method
